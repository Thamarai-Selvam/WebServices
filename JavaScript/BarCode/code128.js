...
exports.Code128 = function() {
    function Code128(input) {
        ...
        if (/^[0-9]{1}$/.test(input)) {
            start = 103;
            elements = chars.map(function(item) { return ELEMENT_TABLE.find(function(o) { return o[1] === item; }); });
        } else if (/^[0-9]+$/.test(input) && input.length % 2 === 0) {
            start = 105;
            var _loop = function _loop(i, len) {
                var key = '' + chars[i] + chars[i + 1];
                elements.push(ELEMENT_TABLE.find(function(o) { return o[3] === key; }));
            };
            for (var i = 0, len = chars.length; i < len; i += 2) { _loop(i, len); }
            ...
            for (var i = 0, len = chars.length - 1; i < len; i += 2) { _loop2(i, len); }
            elements.push(ELEMENT_TABLE[101]);
            elements.push(ELEMENT_TABLE.find(function(o) { return o[1] === chars[chars.length - 1]; }));
        } else if (/^[A-Z0-9]+$/.test(input)) {
            start = 103;
            elements = chars.map(function(item) { return ELEMENT_TABLE.find(function(o) { return o[1] === item; }); });
        } else {
            start = 104;
            elements = chars.map(function(item) { return ELEMENT_TABLE.find(function(o) { return o[2] === item; }); });
        }
        var checkSum = 0;
        elements.forEach(function(item, i) { checkSum += item[0] * (i + 1); });
        checkSum += start;
    ...
    createClass(Code128, [{
        ...
        value: function insert() {
            ...
            var units = this.elements.map(function(item) { return item[5]; }).join('').split('');
            var uw = 1;
            var uh = uw * 50;

            var canvas = createCanvas(units.length * 1.2, uh + 20)
            ...
            for (var i = 0, len = units.length; i < len; i++) { if (+units[i]) { drawUnit(context, i, uw, uh, '#000'); } else { drawUnit(context, i, uw, uh, '#fff'); } }
            // target.appendChild(canvas);
            console.log(this.inPput, this.input)
            context.fillText(this.inPput, units.length / 3, uh + 10)

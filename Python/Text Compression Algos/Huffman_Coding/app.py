...
def compress_file(input_filename, output_filename: str):
    # with open(input_filename, 'rb') as input_file:
        input_bytes = bytearray(input_filename.read())
        ...
        with open(output_filename, 'wb+') as output_file:
            ...
            opFileSize = len(bin_string_to_bytearray(output_binary_string))
        print("Size",len(input_bytes), opFileSize)
        return len(input_bytes), opFileSize


def decompress_file(input_filename: str, output_filename: str):
    # with open(input_filename, 'rb') as input_file:
        # Read the file and convert the bytes to binary strings
    input_bytes = "".join([dec_to_bin(byte, 8) for byte in bytearray(input_filename.read())])
    output_binary_string = ''

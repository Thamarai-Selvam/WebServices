...
def compresscd(data: AnyStr) -> bytes:
...
def decompresscd(data: AnyStr) -> bytes:
    if isinstance(data, str):
        data = data.encode()
...
def compress(data, opFileName):
    # Compresses "example file.txt" into "example file.txt.lzw".
    with open(opFileName, 'wb') as compressed_file:
        fContents = data.read()
        nfContents = compresscd(fContents)
        compressed_file.write(nfContents)
        opFileSize = len(nfContents)
    print("Size",len(fContents), opFileSize)
    return len(fContents), opFileSize

def deCompress(data, opFileName):
    # Decompresses and prints "example file.txt.lzw" content.
    with open(opFileName, 'wb') as unCompressed_file:
        fContents = data.read()
        dcContents = decompresscd(fContents)
        print(dcContents)
        unCompressed_file.write(dcContents)
        opFileSize = len(dcContents)
    print("Size",len(fContents), opFileSize)
    return len(fContents), opFileSize